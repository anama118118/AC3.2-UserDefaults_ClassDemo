...
class ViewController: UIViewController {
    ...
    // MARK: - In-Class Paired Demo
    internal func deleteDefault() {
        let defaults = UserDefaults.standard
        defaults.set(nil, forKey: "instaCats")
        
        
        if defaults.dictionary(forKey: "instaCats") != nil {
            defaults.removeObject(forKey: "instaCats")
        }
        ...
        if let defaultsCatDict = defaults.value(forKey: "instaCats") as? [String : String] {
            ...
            for i in 0..<3 {
                ...
                guard let name = defaultsCatDict["name\(i)"],
                    ...
                    let intId = Int(id) else {return}
                    appendMeow.append(InstaCat(name: name, id: intId, instagramURL: instagramURL))
            }
            print(appendMeow)
            if let testCat = defaultsCatDict["name0"] {
                //print("hello cat" + testCat)
            }
        }
    }
